...
```
代码基线 = 6bcd28f        本文档自身入库提交 = 1763d49   工作区干净
origin/main 与本地同步
```

> 说明：上一轮的最后 4 个提交里有 3 个是纯文档/脚本（`f8bf919`、`6bcd28f`、`1763d49`），
> **被测代码自 `a9232c7` 起未变**。所以 §3.1 的所有实测数字仍然对当前 HEAD 有效。
...
### 3.1 验收数字（全部 0 failures；数字来自 `a9232c7` 的代码，而代码自那以后未变）
