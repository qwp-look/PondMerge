...
rather than 64 B and the per-object term differs accordingly — about **90 B**
per object (52 descriptor + 38 plan scratch; the address-order scratch landed
in v17) instead of 98 B. Measured device configuration: **256 objects / 16
pools → 30,729 B** (2026-09-25 acceptance firmware). **For a RAM budget, read
the runtime `global_stats().metadata_bytes`** — it is an exact upper bound on
every ABI; the closed formula is an illustration, not a contract.

All six rows below were re-measured on 2026-09-25 (x86-64, g++ 15.2, Release):
the closed formula reproduces every one of them.

| `PM_MAX_OBJECTS` | `PM_MAX_POOLS` | `metadata_bytes` (measured) | suits |
|---|---|---|---|
| 64 | 2 | 7,812 | very small targets |
| 128 | 4 | 15,548 | small MCU |
| 256 | 4 | 29,116 | small MCU (recommended start) |
| **256** | **16** | **34,828** | **the ESP32-S3 acceptance firmware (30,729 B measured on the part)** |
| 512 | 8 | 58,156 | medium |
| 1024 | 16 | 116,236 | default (host / large-memory targets) |

**The default costs about 113.5 KiB (116,236 B) of static RAM, which most MCUs
cannot afford.** At `PM_MAX_OBJECTS=256` the table is roughly 34 KiB on x86-64
and **30 KiB on a 32-bit target** (30,729 B measured on the ESP32-S3). Treat
`metadata_bytes` as the budget's upper bound: the linker may place a few KiB
less (GCC section placement measurably differs at 512/8 and 1024/16), and
over-estimating is the safe direction. (Before v1.0.0 that figure silently
omitted the advice cache, under-reporting by 128 B at 2 pools up to 960 B at
16 pools. It is included now; the pre-2026-09-25 table rows that predate the
v17 scratch were stale and have been re-measured.)
...
```cmake
set(PM_MAX_OBJECTS 256)   # ~113.5 KiB -> ~30 KiB on a 32-bit target (30,729 B
                          # measured on ESP32-S3; ~34 KiB on x86-64)
